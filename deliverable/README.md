...
### 01-source-code.zip
- `README.txt` — **hướng dẫn chạy** (Arduino + Pi + Bluetooth) + sơ đồ chân
- `arduino/greenhouse/greenhouse.ino` — code Arduino (DHT22, LDR, LED, Servo bit-bang, Buzzer, LCD, HC-05)
- `arduino/servo_test/servo_test.ino` — sketch test servo (debug)
- `rpi/app.py` — Flask + SQLite + đọc serial (Bluetooth ưu tiên, USB backup)
- `rpi/templates/index.html` — dashboard
- `rpi/telegram_notify.py` + `telegram_config.example.json` — cảnh báo Telegram
...
- `lcd-fire-alarm.jpg` — ảnh mạch thật: LCD báo **"!! FIRE ALARM !"** + VENT OP (chống cháy)
- `lcd-normal.jpg` — ảnh mạch thật: LCD hiển thị T/H, chế độ MANUAL
- `telegram-alerts.jpg` — ảnh thật kênh Telegram **TheHouse**: cảnh báo cháy + "Đã an toàn"
...
## Trạng thái nộp bài
✅ **Đã đủ bộ deliverable**: source + slide PDF + screenshot dashboard + ảnh mạch thật + ảnh Telegram + sơ đồ đấu nối + video demo + playbook.
